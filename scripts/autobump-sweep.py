...
#
# Strip TOML header trailing comments before exact matching, or annotated opted-in packages are silently skipped.
...
# The engine's temporary directory follows TMPDIR, so hard-coding /tmp loses evidence on Gentoo systems.
...
def evidence_file_excerpt(path):
    ...
    lines = [
        line.replace("```", "")
        for line in text.split("\n")
        if not re.fullmatch(r"[ \t\r\v\f]*", line)
    ]
...
def evidence_excerpt(evidence):
    ...
    try:
        ...
        return ""
    paths.sort(key=lambda path: path.name != "escalations.txt")
    ...
    for path in paths:
        ...
        if len(section) > budget:
            sections.append(f"({len(paths) - len(sections)} more files in the run's evidence artifact)")
            break
...
def copy_tools():
    # Copy helpers before branch switching so a helper present only on the starting branch cannot disappear mid-sweep.
    ...
    return {
        "matrix": {
            "include": [
                {
                    "shard": index,
                    # the packages this shard carries, so the job reads as a name and not
                    # as the JSON payload GitHub would otherwise title it with
                    "packages": " ".join(item["package"] for item in shard),
                    "items": json.dumps({"items": shard}, separators=(",", ":")),
                }
...
def run_issues(settings, issues, apply_run_limit, tools, engine):
    ...
    for issue in issues:
        ...
        attempts += 1
        # The replaced shell ran without set -e; one broken issue must not cost remaining packages their turn or summary.
        try:
