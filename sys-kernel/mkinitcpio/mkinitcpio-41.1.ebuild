...
SRC_URI="https://gitlab.archlinux.org/archlinux/mkinitcpio/mkinitcpio/-/archive/v${PV}/mkinitcpio-v${PV}.tar.bz2 -> ${P}.tar.bz2"
...
src_prepare() {
	default
	sed -i "s/version: run_command('tools\/dist.sh', 'get-version', check: true).stdout().strip()/version: '${PV}'/" \
		meson.build || die
