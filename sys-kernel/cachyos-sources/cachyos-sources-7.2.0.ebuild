...
apply_gentoo_genpatches() {
	...
	# Gentoo fixes below were written against a different tree. A x.y.0 release
	# has no stable increment yet, so genpatches legitimately carries none.
	if [[ ${KV_PATCH} -ne 0 ]]; then
