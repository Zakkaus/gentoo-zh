...
- A dist-kernel package carries no `PDEPEND` on `virtual/dist-kernel` and this overlay ships no `virtual/dist-kernel`. `>=` lets Portage satisfy it from `::gentoo` and install a second kernel, `=` pins a `SLOT=0` virtual to one version so two of these kernels cannot coexist, and an overlay copy has to track `::gentoo`'s provider versions, which a kernel on its own version scheme such as `sys-kernel/xanmod-rt` can never match. Warn in `pkg_postinst` that external modules need `eselect kernel`, `emerge @module-rebuild` and `emerge --config` instead.
