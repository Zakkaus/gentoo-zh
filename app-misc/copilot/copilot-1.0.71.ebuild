...
# Upstream's @github/copilot package is a thin loader that execs the per-platform
# native binary shipped in @github/copilot-<os>-<arch>; install that binary directly.
...
src_install() {
	...
	# The bundled webview addon links libxdo.so.3 (Gentoo ships libxdo.so.4) and webkit;
	# it cannot load here. Drop the native binding rather than ship it broken — the CLI's
	# device-code login works without the browser webview.
