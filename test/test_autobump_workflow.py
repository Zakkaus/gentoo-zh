...
class AutobumpWorkflowContractTest(unittest.TestCase):
        ...
        for job, step in driver_steps():
            for line in step["run"].splitlines():
                if "autobump-sweep.py" not in line:
                    continue
                # ${{ }} and $VAR are values, not flags; shlex keeps them as single words
    ...
    def test_the_delta_a_worker_writes_is_the_one_collect_downloads(self):
        ...
        # the two sides name the shard differently ($SHARD_ID in the shell, matrix.shard in the
        # YAML); compare the shape around it
